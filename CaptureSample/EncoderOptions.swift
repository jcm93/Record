...
public struct Options: @unchecked Sendable {
    ...
    func logStart(_ logger: Logger) {
        ...
        Encoder session started with options:
            ...
            codec             : \((codec as FourCharCode).fourCCDescription, privacy: .public)
            dimensions        : \(destWidth) x \(destHeight)
            keyframe-duration : \(formatter.string(from: TimeInterval(maxKeyFrameIntervalDuration))!, privacy: .public)
            keyframe-interval : \(maxKeyFrameInterval) frames
            out               : \(outputFolder, privacy: .private(mask: .hash))
            pixel-format      : \((pixelFormat as FourCharCode).fourCCDescription, privacy: .public)
            rate-control      : \(rateControl.rawValue, privacy: .public)
            icc-profile       : \(iccProfile.debugDescription)
            bit-depth         : \(bitDepth)
            ...
            replay-duration   : \(formatter.string(from: TimeInterval(replayBufferDuration))!, privacy: .public)
        """)
    }
}
